Shader "Custom/FogGauss"
{
    Properties
    {
        _MainTex ("Base (RGB), Alpha (A)", 2D) = "black" {}
        _TexSize("Texture Size", vector) = (256,256,0,0)
        _BlurOffset("Blur Offset", Range(0.1, 10)) = 1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 filter(float3x3 filter, sampler2D tex, float2 coord, float2 texSize)
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    for (int j = 0; j < 3; j++)
                    {
                        ...
                        float2 newCoord = float2(coord.x + (i-1)*_BlurOffset * _FogIntensity, coord.y + (j-1)*_BlurOffset * _FogIntensity);
            ...
            fixed4 frag (v2f IN) : COLOR
            {
                ...
                blur.rgb = _FogIntensity * _FogColor + (1 - _FogIntensity) * blur.rgb;
                return blur;
            }
            ENDCG
        }
